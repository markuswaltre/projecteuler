...
for (var i = 0; i < size; i++) {
	for(var j = 0; j < size; j++) {
		// right
		if((j+3) < size) {
			sum = lines[i][j] * lines[i][j+1] * lines[i][j+2] * lines[i][j+3];
			if(sum > biggest) biggest = sum;
		}

		// down
		if((i+3) < size) {
			sum = lines[i][j] * lines[i+1][j] * lines[i+2][j] * lines[i+3][j];	
			if(sum > biggest) biggest = sum;
		}

		// diagonal right
		if((i+3) < size && (j+3) < size) {
			sum = lines[i][j] * lines[i+1][j+1] * lines[i+2][j+2] * lines[i+3][j+3];	
			if(sum > biggest) biggest = sum;
		}
